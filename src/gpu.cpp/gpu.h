    ...
    const WGPUDeviceDescriptor &devDescriptor = {}) 
{
  ...
  LOG(kDefLog, kTrace, "Requesting adapter");
  {
    ...
    WGPURequestAdapterCallbackInfo callbackInfo {
      ...
      .userdata = &adapterData
    };
    wgpuInstanceRequestAdapter(ctx.instance, &adapterOpts, callbackInfo.callback, callbackInfo.userdata);
  ...
  LOG(kDefLog, kTrace, "Requesting device");
  {
    ...
    wgpuAdapterRequestDevice(ctx.adapter, &devDescriptor, deviceCallbackInfo.callback, deviceCallbackInfo.userdata);
      ...
      .callback =
          [](WGPUQueueWorkDoneStatus status, void *userdata) {
            ...
            WGPUBufferMapCallbackInfo mapCallbackInfo = {
                ...
                .userdata = const_cast<CallbackData *>(data),};
            wgpuBufferMapAsync(data->buffer, WGPUMapMode_Read, 0,
                               data->bufferSize, mapCallbackInfo.callback, mapCallbackInfo.userdata);
          },
      .userdata = &callbackData};
  wgpuQueueOnSubmittedWorkDone(ctx.queue, workDoneCallbackInfo.callback, workDoneCallbackInfo.userdata);
...
inline Kernel createKernel(Context& ctx, const KernelCode &code,
    ...
    if (compilationInfo && result) {
      ...
      for (uint32_t i = 0; i < compilationInfo->messageCount; ++i) {
        ...
        result->messages.push_back(
            std::string(compilationInfo->messages[i].message));
  ...
  WGPUCompilationInfoCallbackInfo compilationCallbackInfo = {
      ...
      .userdata = static_cast<void *>(compilationInfo)};
  ...
  WGPUQueueWorkDoneCallbackInfo workDoneCallbackInfo = {
      ...
      .callback =
          [](WGPUQueueWorkDoneStatus status, void *userdata) {
            check(status == WGPUQueueWorkDoneStatus_Success, "Queue work done",
                  __FILE__, __LINE__);
            auto *promise = static_cast<std::promise<void> *>(userdata);
            promise->set_value();
          },
      .userdata = &promise};
  wgpuQueueOnSubmittedWorkDone(ctx.queue, workDoneCallbackInfo.callback, workDoneCallbackInfo.userdata);
